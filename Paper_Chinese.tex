...
\subsubsection{机器状态}
机器状态 $\boldsymbol{\mu}$ 定义为一个元组 $(g, pc, \mathbf{m}, i, \mathbf{s})$ , 包括可用的燃料 $g$, 程序计数器 $pc \in \mathbb{P}_{256}$ , 内存内容 \mathbf{m} , 内存中激活的字数(从位置 0 开始连续计数) $i$,以及栈内容 \mathbf{s}。内存内容 $\boldsymbol{\mu}_\mathbf{m}$ 表示空间大小为 $2^{256}$ 但内容都是0的空间。
...
\subsubsection{跳转目的有效性}

跳转目的地的有效性（“Jump Destination Validity”）。我们之前使用D作为一个函数去决定给定
